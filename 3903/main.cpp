...
bool f(vector<HalfPlane> &edge, double x) {
    vector<HalfPlane> tmp;
    for (int i = 0; i < edge.size(); i++) {
        double a = edge[i].d.x, b = edge[i].d.y;
        Point n = Point(-b, a) * (1 / sqrt(a*a+b*b));
        if (cross(edge[i].d, n) < 0) n = n*(-1);
        n = n*x;
        HalfPlane HP(edge[i].p + n, edge[i].p + n + edge[i].d);
        tmp.push_back(HP);
    }
    if (HP_inter(tmp).empty()) return 1;
    return 0;
}
