...
struct Node {
    ll lmx, rmx, mx, sum;
    Node() {lmx = rmx = mx = -INF; sum = 0;}
    Node(ll lmx, ll rmx, ll mx, ll sum): lmx(lmx), rmx(rmx), mx(mx), sum(sum) {}
    Node operator + (const Node& node) const {
        return {max<ll>({lmx, sum + node.lmx}), max<ll>({node.rmx, node.sum + rmx}),
                max<ll>({mx, node.mx, rmx + node.lmx}), sum + node.sum};
    }
    void set(ll v) {lmx = rmx = mx = sum = v;}
};
...
struct SegTree {
    ...
    void update_lazy(int node, int s, int e) {
        ...
        tree[node].set(max<ll>((e-s+1)*lazy[node], lazy[node]));
